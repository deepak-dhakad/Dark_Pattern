...
function scrape() {
  ...
  for (let i = 0; i < elements.length; i++) {
    let texts = elements[i].innerText.trim().split(/\r?\n/);
    for (let text of texts) {
      ...
      // Check for the specific pattern and highlight if found
      if (extractValues(text)) {
        highlight(elements[i], "Price Pattern");
        dp_count++;
      }
    }
  }
